...
-- Create candidate soft skills table (drop and recreate to ensure clean state)
DROP TABLE IF EXISTS public.candidate_soft_skills CASCADE;
...
DROP POLICY IF EXISTS "Candidates can view own matches" ON public.job_matches;
DROP POLICY IF EXISTS "Employers can view matches for their jobs" ON public.job_matches;
DROP POLICY IF EXISTS "System can manage matches" ON public.job_matches;
...
DROP TRIGGER IF EXISTS update_job_matches_updated_at_trigger ON public.job_matches;
